...
listing_mod <- listings_raw %>% 
  ...
  mutate_at(vars(bathrooms, beds, zipcode, bedrooms, guests_included, cleaning_fee),
            funs(as.factor)) %>% 
  group_by(zipcode) %>%
...
conf_reg <- conf_matrix_reg %>% 
  ...
  labs(title = 'Mastriz de confusión (regresión)')
...
conf_gbm <- conf_matrix_gbm %>% 
  ...
  theme_minimal() +
  theme(legend.position = 'none') +
  labs(title = 'Mastriz de confusión (gbm)')
...
conf_ET <- conf_matrix_ET %>% 
  ...
  theme_minimal() +
  theme(legend.position = 'none') +
  labs(title = 'Mastriz de confusión (extraTrees)')
